...
public class ModifyStudent extends JFrame{
	...
	public ModifyStudent(String da[],Socket socket) 
	{
		super("修改学生信息");
		this.socket=socket;
		try {
	    ...
	    b1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				//	String depart1= tf7.getText().trim();			
						String sql="update Student set sid='"+sid1+"',password='"+password1+"',name='"+name1+"',birthday='"+birthday1+"',ssn='"+ssn1+"',status='"+status1+"' where sid='"+sid+"'";   //SQL语句
						try {
							dos.writeUTF("33"+"#"+sql);
							dos.flush();
