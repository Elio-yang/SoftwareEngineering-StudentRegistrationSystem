...
public class Professor {
    ...
    public ResultSet findQualifiedCourses() {//检索有资格教授的课程，直接検索course表，里面全是这学期开的课程，即教授有资格教授的课程，但是要除去id是自己的
        ...
        try {
            ...
            sql = "select * from course where pid is not null";//查询有资格教授的课程，就是pid为空的那些，，但是显示原因，只能用空格，pid不是自己的就是别人的
            var pstmt = conn.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
            rs = pstmt.executeQuery(sql);//返回查询结果
    ...
    public ResultSet preTaughtCourses(String pid) {//检索以前教授过的课程，参数为教授的id
        ...
        try {
            ...
            sql = "select cid,cname,credit,semester from grade where pid='" + pid + "' ";//查询语句
            var pstmt = conn.prepareStatement(sql, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);
            rs = pstmt.executeQuery(sql);//返回查询结果
        } catch (SQLException e) {
            // TODO Auto-generated catch block
